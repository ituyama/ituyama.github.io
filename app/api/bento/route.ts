...
- time: 日本時間のライブ時計。body は不要（自動で現在時刻を表示）。「今何してる？」等の時間に関する関心に応えるとき使う。
- music: Spotify プレーヤー。body = 事実の Spotify URL。事実に Spotify がある場合のみ使う。
- aquarium: 金魚が泳ぐ癒しの装飾タイル。body 不要。余白の彩りとして 1 枚だけ添えてよい。
- dvd: 跳ね回る DVD ロゴのスクリーンセーバー。body 不要。遊び心の装飾として 1 枚だけ添えてよい。
- stock: どうぶつの森風「カブ価」タイル。日経平均に連動したベル価格を表示。body 不要。マーケットや投資、ゲームに関心がある訪問者に使う。
- tweets: X（Twitter）プロフィールへのカード。body = 事実の X ハンドル（例: "@PM80437319"）、href = 事実の X プロフィール URL。SNS や近況に関心がある訪問者に使う。
